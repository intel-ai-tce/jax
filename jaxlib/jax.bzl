...
def _get_full_wheel_name(rule_name, platform_name, cpu_name, major_cuda_version, wheel_version):
    if "pjrt" in rule_name:
        wheel_name_template = "{package_name}-{wheel_version}-SNAPSHOT-py{major_python_version}-none-{wheel_platform_tag}.whl"
    else:
        wheel_name_template = "{package_name}-{wheel_version}-SNAPSHOT-cp{python_version}-cp{python_version}-{wheel_platform_tag}.whl"
...
def _jax_wheel_impl(ctx):
    ...
    wheel_name = _get_full_wheel_name(
        ...
        ctx.attr.wheel_version,
    )
    output_file = ctx.actions.declare_file(ctx.label.name + "/" + wheel_name)
    ...
    args["--jaxlib_git_hash"] = jaxlib_git_hash  # required argument
